...
func (i *InstaData) GetData() error {
	cacheInstaData, closer, err := DB.Get(utils.S2B(i.PostID))
	if err != nil && err != pebble.ErrNotFound {
		log.Error().Str("postID", i.PostID).Err(err).Msg("Failed to get data from cache")
		return err
	}
	...
	if len(cacheInstaData) > 0 {
		...
		log.Debug().Str("postID", i.PostID).Msg("Data parsed from cache")
		return nil
	}
